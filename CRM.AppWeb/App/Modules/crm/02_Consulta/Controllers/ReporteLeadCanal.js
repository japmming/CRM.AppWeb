define([$appName,
    "ReporteLeadCanal", ["CAMPANIA", "EVENTO", "LIST_USUARIO_NOMBRE", "LIST_EJECUTIVOS_CON_LEADS","LIST_USUARIO_ASESOR_SUPERVISOR"]
], function (module, namespace, args) {
    "use strict";
    var service = function ($service, $http) {
        $service.CoMetrLista = function (item, callback) {
            return $http({
                url: "ReporteLeadCanal/ListarReporteCanal",
                data: item,
                method: "post",
                callback: callback
            });
    ...
    var controller = function ($scope, $service) {
                ...
                if (d.success) {
                    if (d.data != '') {
                        ...
                        if (listas.length > 0) {
                            ...
                            $$grid('grd_repCanal').setData(listaUsuarios);
                            //console.log(listas);

                        }
                    } else {
                        $$grid('grd_repCanal').clearGrid();
                    }
        ...
        $scope.cargarCombos = function () {
            ...
            //var ListaUsuarios = $global.lists.LIST_USUARIO_ASESOR_SUPERVISOR.slice() //LIST_USUARIO_NOMBRE;
           /* ListaUsuarios.push('SIN ASIGNAR' + '¦' + 'SIN ASIGNAR');*/
            //console.log($global.lists.LIST_USUARIO_ASESOR_SUPERVISOR)
            $fnSetCombo({
                controlId: 'cbo_LeadsNombre_Ejecutivo',
                data: $global.lists.LIST_USUARIO_ASESOR_SUPERVISOR, //$global.lists.LIST_EJECUTIVOS_CON_LEADS,
                type: 'T',
                separator: '¦'
            });
        };
