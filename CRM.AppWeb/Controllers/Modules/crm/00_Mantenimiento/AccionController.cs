    ...
    public class AccionController : BaseController
    {
        public async Task<string> Listar(string data)
        {
            ...
            if (!string.IsNullOrEmpty(data))
            {
                ...
                response = await odaSQL.ExecuteCommand("dbo.CSV_ACCION_LISTAR_SP", data);
            }
            return Ok(response);
        }

        public async Task<string> CUD_Accion(string data)
        {
            ...
            {
                DataSQL odaSQL = new DataSQL();
                response = await odaSQL.ExecuteCommand("dbo.CSV_ACCION_CUD_SP", data);
            }
            return Ok(response);
        }
