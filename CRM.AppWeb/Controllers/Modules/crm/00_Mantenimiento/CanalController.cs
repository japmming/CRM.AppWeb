    ...
    public class CanalController : BaseController
    {
        public async Task<string> Listar(string data)
        {
            ...
            if (!string.IsNullOrEmpty(data))
            {
                ...
                response = await odaSQL.ExecuteCommand("dbo.CSV_CANAL_LISTAR_SP", data);
            }
            return Ok(response);
        }

        public async Task<string> CUD_Canal(string data)
        {
            ...
            {
                DataSQL odaSQL = new DataSQL();
                response = await odaSQL.ExecuteCommand("dbo.CSV_CANAL_CUD_SP", data);
            }
            return Ok(response);
        }
